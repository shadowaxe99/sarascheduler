...
class Intent(_LuisData, collections.namedtuple("_Intent", ["intent", "score"])):
...
class Entity(
    ...
    collections.namedtuple(
        "_Entity", ["entity", "type", "score", "start_index", "end_index", "resolution"]
    ),
):
...
class LuisResult(
    _LuisData, collections.namedtuple("_LuisResult", ["intents", "entities", "query"])
):
    ...
    def _from_json(klass, json):
        ...
        return LuisResult(query=json["query"], intents=intents, entities=entities)
...
class Luis(object):
    ...
    def analyze(self, text):
        ...
        logger.debug("LUIS returned status %s with text: %s", r.status_code, r.text)
        r.raise_for_status()
