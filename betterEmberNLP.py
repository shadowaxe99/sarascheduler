...
def analyze_text():
    ...
    for sentence in xrange(len(sentences)):
        ...
        if find_dates(sentences[sentence]) != [
            ["", []]
        ]:  # if the sentence contains a date and/or a time, do the following:
            ...
            for timestring in xrange(len(time)):
                ...
                if timestring != len(time) - 1:
                    times += ", "  # creates a string with all the times. formats correctly if there is more than one time.
            if date != "":
                datetimeSentences[
                    sentences[sentence]
                ] = date  # if there is a date, create a key and corresponding value in the dictionary
            if str(time) != "[]":
                datetimeSentences[
                    sentences[sentence]
                ] = times  # if there is a time, create a key and corresponding value in the dictionary
...
def find_dates(string):
    ...
    l = luis.Luis(
        url="https://api.projectoxford.ai/luis/v1/application?id=5d787786-7e0e-4055-9009-8eeab0baa48f&subscription-key=c4bfbca7ccc34ca8b0ab120b4a6aa56b"
    )
    r = l.analyze(string)
    for i in xrange(len(r.entities)):
        if "datetime" in str(r.entities[i].type):
            if r.entities[i].resolution.keys() == [u"date"]:
                ...
                tempdates.append(
                    [r.entities[i].resolution.values()]
                )  # gets date values    ''' tempdates.append([r.entities[i].entity])
    ...
    for j in xrange(len(tempdates)):
        ...
        if is_int(readday) and day > int(readday):
            if month == 12:
                ...
                month += 1
        if readmonth != "XX" and int(readmonth) < month:
            year += 1
        if readyear == "XXXX":
            readyear = year
        if readmonth == "XX":
